...
def evaluate(preds, gts, language='en'):
    if language == 'en':
        nlp = spacy.load("en_core_web_sm")
        split_function = lambda x: [x.text for x in nlp(x)]
    elif language == 'ch':
        split_function = jieba.cut
    else:
        raise ValueError
    ...
    for gt, pred in tqdm(zip(gts, preds)):
        pred_list = split_function(pred.strip())
        # pred_list = pred.split()

        pred_bleu.append(pred_list)

        gt_list = [split_function(y.strip()) for y in gt]
...
def evaluate_list(preds, gts, language='en'):
    if language == 'en':
        nlp = spacy.load("en_core_web_sm")
        split_function = lambda x: [x.text for x in nlp(x)]
    elif language == 'ch':
        ...
        raise ValueError
    res = {'R2': [], 'RL': [], 'B1': [], 'B2': [], 'B3': [], 'B4': [], "D1": [], "D2": [], "D3": [], "D4": []}
    if type(gts[0]) == str:
        gts = [[x] for x in gts]
    assert len(preds) == len(gts)
    ...
    for idx in range(len(pred_bleu)):
        res["D1"].append( distinct_ngrams(pred_bleu[idx: idx + 1], 1))
        res["D2"].append( distinct_ngrams(pred_bleu[idx: idx + 1], 2))
        res["D3"].append( distinct_ngrams(pred_bleu[idx: idx + 1], 3))
        res["D4"].append( distinct_ngrams(pred_bleu[idx: idx + 1], 4))

        res['B4'].append(_compute_bleu(gt_bleu[idx: idx + 1], pred_bleu[idx: idx + 1], max_order=4)[0])
        res['B3'].append(_compute_bleu(gt_bleu[idx: idx + 1], pred_bleu[idx: idx + 1], max_order=3)[0])
        res['B2'].append(_compute_bleu(gt_bleu[idx: idx + 1], pred_bleu[idx: idx + 1], max_order=2)[0])
        res['B1'].append(_compute_bleu(gt_bleu[idx: idx + 1], pred_bleu[idx: idx + 1], max_order=1)[0])

    # print(
    #     'Bleu-1: {:.4f} | Bleu-2: {:.4f} | Bleu-3 {:.4f} | Bleu-4 {:.4f} | Dist-1 {:.4f} | Dist-2 {:.4f} | Dist-3 {:.4f} | Dist-4 {:.4f}'.format(
    #         res['B1'], res['B2'], res['B3'], res['B4'],
    #         res['D1'], res['D2'], res['D3'], res['D4']), file=rl_f)
    # rl_f.close()
    return res

def evaluate_file(GT_FILE, GEN_FILE, language):
    preds = read(GEN_FILE)
    gts = read_reference(GT_FILE)
    return evaluate(preds, gts, language)


def compare(GEN_FILE1, GEN_FILE2, history_story_file):
    nlp = spacy.load("en_core_web_sm")
    preds1 = read(GEN_FILE1)
    preds2 = read(GEN_FILE2)
    gts = read_reference(GT_FILE)
    if history_story_file.endswith('.csv'):
        history_story = read_reference(history_story_file)
    else:
        history_story = json.load(open(history_story_file, 'r', encoding='utf8'))
    gt_bleu = []
    pred1_bleu = []
    pred2_bleu = []
    for gt, pred1, pred2 in tqdm(zip(gts, preds1, preds2)):
        pred1_list = [x.text for x in nlp(pred1.strip())]
        pred2_list = [x.text for x in nlp(pred2.strip())]
        # pred_list = pred.split()

        pred1_bleu.append(pred1_list)
        pred2_bleu.append(pred2_list)
    ...
    with open('compare.txt', 'w', encoding='utf8') as f:
        for i in range(len(gt_bleu)):
            bleu1 = _compute_bleu(gt_bleu[i: i + 1], pred1_bleu[i: i + 1], 1)[0]
            bleu2 = _compute_bleu(gt_bleu[i: i + 1], pred2_bleu[i: i + 1], 1)[0]
            if bleu2 - bleu1 > 0.3:
            # if bleu1 > 0 or bleu2 > 0:
                print(f'#{i}: bleu1: {bleu1}, bleu2: {bleu2}', file=f)
                for sentence in history_story[i]['story']:
                    print(sentence, file=f)
                print('', file=f)
                print('answer1:', preds1[i], file=f)
                print('answer2:', preds2[i], file=f)
                print('golden_:', gts[i], file=f)
                print('-----------------------------------------------', file=f)
...
if len(sys.argv) == 4:
    ...
    GEN_FILE = sys.argv[2]
    language = sys.argv[3]
    print(evaluate_file(GT_FILE, GEN_FILE, language))
elif len(sys.argv) == 6:
    GT_FILE = sys.argv[1]
    GEN_FILE1 = sys.argv[2]
    GEN_FILE2 = sys.argv[3]
    history_story_file = sys.argv[4]
    language = sys.argv[5]
    compare(GEN_FILE1, GEN_FILE2, history_story_file)
